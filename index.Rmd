...
First of all we need to download the train and test data set from the link provided and below method is how we do it.
```{r DownloadData}
...
```{r SampleError}
rfAccuracy <- as.numeric(confusionMatrix(predRf, as.factor(validateData$classe))$overall['Accuracy'])
sampleError <- 1 - rfAccuracy
sampleError
```

### 5. Conclusion
From the accuracy test that we have carried out above, Random Forest model has higher accuracy of `r rfAccuracy' with estimated expected out of sample error of `r sampleError`. Therefore Random Forest model will be used to predict the test cases.
